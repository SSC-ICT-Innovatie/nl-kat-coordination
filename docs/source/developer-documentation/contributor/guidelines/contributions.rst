...
1. Add a new language through Weblate's interface
    Alternatively: create a new ``openkat/locale/$LANGUAGE_CODE/LC_MESSAGES/django.po`` file directly in the repository.
    You can use ``openkat/locale/django.pot`` as a template.
2. Add the language code to the ``LANGUAGES`` list in ``openkat/settings.py``
