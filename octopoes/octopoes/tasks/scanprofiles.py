...
def scan_profile_recalculations(katalogusclient: KATalogusClient, octopii: dict) -> None:
    ...
    for org in orgs:
        ...
        if last_transaction:
            ...
            # but since we have no database lock we asume we are the only one 
            # writing transations during our scan_profile calculcation.
...
def recalculate_scan_profiles_for_org(session: dict) -> int | None:
    ...
    if max_id and session["last_transaction"] == max_id["txId"]:
        logger.debug("skipping scan profile recalculation task, no new transactions present, last transaction: %i [org=%s]", max_id["txId"], session["org"])
        return None
    else:
        logger.debug("Most recent worked transactions %i, most recent %i [org=%s] ", session["last_transaction"], max_id["txId"],  session["org"],)
    ...
    try:
        ...
        logger.info("Finished scan profile recalculation on %i unproccessed transactions, with resulting transactioncount: %i [org=%s] [dur=%.2fs]", (max_id["txId"] - session["last_transaction"]), transactions, session["org"], duration)
        return max_id["txId"]
    except Exception:
