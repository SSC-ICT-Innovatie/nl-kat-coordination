...
def upgrade():
    ...
    op.create_index('ix_tasks_active_per_schedule', 'tasks', ['schedule_id'], unique=True, postgresql_where=sa.text("status IN ('PENDING', 'QUEUED', 'DISPATCHED', 'RUNNING')"))
...
def downgrade():
    ...
    op.drop_index('ix_tasks_active_per_schedule', table_name='tasks', postgresql_where=sa.text("status IN ('PENDING', 'QUEUED', 'DISPATCHED', 'RUNNING')"))
