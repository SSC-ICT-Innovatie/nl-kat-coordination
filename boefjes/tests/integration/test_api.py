...
def test_filter_plugins(test_client, organisation):
    ...
    response = test_client.get(
        f"/v1/organisations/{organisation.id}/plugins", params={"oci_image": "docker.underdark.nl/librekat/openkat-nmap:latest"}
    )
    assert {x["id"] for x in response.json()} == {"nmap", "nmap-ip-range", "nmap-udp", "nmap-ports"}

    boefje = Boefje(
        id="test_plugin", name="My test boefje", static=False, oci_image="docker.underdark.nl/librekat/openkat-nmap:latest"
    )
