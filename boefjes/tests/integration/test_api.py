...
def test_filter_plugins(test_client, organisation):
    ...
    response = test_client.post(f"/v1/organisations/{organisation.id}/plugins", json=boefje.model_dump())
    assert response.status_code == 201
...
def test_cannot_add_plugin_reserved_id(test_client, organisation):
    ...
    response = test_client.post(f"/v1/organisations/{organisation.id}/plugins", json=normalizer.model_dump)
    assert response.status_code == 400
    assert response.json() == {"detail": "Duplicate plugin: a plugin with this id already exists"}
...
def test_update_plugins(test_client, organisation, second_organisation):
    ...
    test_client.post(f"/v1/organisations/{organisation.id}/plugins", json=boefje.model_dump())
    ...
    test_client.post(f"/v1/organisations/{organisation.id}/plugins", json=normalizer.model_dump())
...
def test_clone_settings_and_config_api_shows_both(test_client, organisation):
    ...
    test_client.post("/v1/organisations/", json=org2.model_dump())
    ...
    test_client.put("/v1/organisations/", json=org2.model_dump())
